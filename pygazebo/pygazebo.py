...
class _Connection(object):
    ...
    def connect(self, address):
        ...
        future = create_task(loop.sock_connect(self.socket, address))
    ...
    def start_accept(self, callback):
        loop = asyncio.get_event_loop()
        future = create_task(loop.sock_accept(self.socket))
        future.add_done_callback(
    ...
    def read_raw(self):
        ...
        loop = asyncio.get_event_loop()
        future = create_task(loop.sock_recv(self.socket, 8))
        future.add_done_callback(
    ...
    def send_pieces(self, data, result=None):
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            future = create_task(loop.sock_sendall(self.socket, this_send))
            future.add_done_callback(
